...
Если пользователь не может выполнить задачу, он может воспользоваться аналогом команды `sudo` - `su`. Команда `su` позволяет пользователю сменить свой идентификатор пользователя на идентификатор суперпользователя без необходимости ввода пароля суперпользователя. Однако это решение не является безопасным, поскольку оно открывает возможность для других пользователей использовать привилегии суперпользователя.
